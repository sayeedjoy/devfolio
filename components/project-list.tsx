  ...
  return (
    <ul className="flex flex-col">
      {projects.map((project) => {
        const { title, subtitle, icon } = project.metadata
        return (
          <li key={project.slug}>
            <Link
              href={`/project/${project.slug}`}
              className="group -mx-2 flex items-center gap-4 rounded-xl px-2 py-3 transition-colors hover:bg-foreground/[0.03]"
            >
