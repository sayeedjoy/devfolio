...
export type ExperienceItemType = {
  ...
  companyLogo?: string
  /**
   * Path to a local logo image under `/public` (e.g. "projects/seu.png").
   * Used as the company mark when `companyLogo` is not set — handy for
   * schools/orgs whose assets live in the repo rather than a remote URL.
   */
  icon?: string
  /** URL to the company's website. */
  companyWebsite?: string
  /**
   * Path under `/public` to a screenshot (e.g. "/previews/foo.png"). When set,
   * hovering the company header parks it in the page gutter via
   * `<HoverPreview />`. Bind it per company — nesting one per position would
   * open two cards at once, since pointer-enter fires on ancestors too.
   */
  preview?: string
  /**
...
export function ExperienceItem({ experience }: ExperienceItemProps) {
  // Prefer a remote `companyLogo`; fall back to a local `icon` asset, ensuring
  // it's root-relative so it resolves from /public regardless of the route.
  const logoSrc =
  ...
  return (
    <div className="space-y-4 py-4">
      <HoverPreview src={experience.preview} className="not-prose w-fit">
...
function Skill({ name, className }: { name: string; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-md border bg-muted/50 px-1.5 py-0.5 font-mono text-xs text-muted-foreground",
        className
      )}
    >
      {/* Renders the brand mark when `name` is a known stack; otherwise null. */}
      <StackIcon name={name} size={12} />
      {name}
    </span>
  )
}
