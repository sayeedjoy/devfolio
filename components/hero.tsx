...
import content, { type BioParagraph } from "@/data/content"
...
function BioLine({ segments }: { segments: BioParagraph }) {
  return (
    <p>
      {segments.map((seg, i) => {
        ...
        if (seg.type === "link") {
          return (
            <a
              ...
              className="font-semibold whitespace-nowrap text-foreground transition-colors hover:text-foreground/70"
            >
              {seg.text}
              {seg.logo ? (
                <Image
                  ...
                  width={16}
                  height={16}
                  className="ml-1 inline-block size-[1.05em] rounded-[3px] object-cover align-[-0.18em]"
                />
              ) : null}
            </a>
          )
